...
public class Test_DynamicGoalStructure {
	...
	static Goal mk_goal1() {
		Tactic increase = action("incr").do1((MyState st) -> { st.x++ ; return st ; }).lift() ;
		return goal("g1").toSolve((MyState st) -> st.x > 2).withTactic(increase) ;
	}
	
	static Goal mk_goal2() {
		Tactic decrease = action("decr").do1((MyState st) -> { st.x-- ; return st ; }).lift() ;
	...
	public void test_IFELSE_goalstructure() {
		...
		// attach a state and environment; the environment is here just a dummy that does not do anything 
	...
	/**
	 * Test WHILE(p,g) for the scenario where the loop terminates because the guard becomes false.
	 */
	@Test
