...
public class TestWithRemotingEnv_GCDGame {
     ...
     * 
     * var state = (MyState) (new MyState().setEnvironment(new GCDEnv())) ; var
     * agent=new TestAgent().attachState(state);
     * 
     * //Setting goal and action var a0 = action("a0") . do1((MyState S)-> {
     * S.result=((GCDEnv)S.env()).Test_GCD(12, 0); return S.result; }) . lift() ;
     * 
     * var topgoal = goal("g").toSolve((Integer k) -> k==12) . withTactic(a0) .
     * lift() ; agent.setGoal(topgoal); agent.update(); assertTrue(state.result==12)
     * ; assertTrue(topgoal.getStatus().success()) ; assertTrue(agent.goal == null)
     * ;
     * 
     * 
     * }
     ...
     * 
     * //Setting goal and action var a0 = action("a0") . do1((MyState S)-> {
     * S.result=((GCDEnv)S.env()).Test_GCD(0, 9); return S.result; }) . lift() ;
     * 
     * var topgoal = goal("g").toSolve((Integer k) -> k==9) . withTactic(a0) .
     * lift() ; agent.setGoal(topgoal); agent.update(); assertTrue(state.result==9)
     * ; assertTrue(topgoal.getStatus().success()) ; assertTrue(agent.goal == null)
     * ;
     * 
     * 
     ...
     * 
     * //Setting goal and action var a0 = action("a0") . do1((MyState S)-> {
     * S.result=((GCDEnv)S.env()).Test_GCD(7966496,314080416); return S.result; }) .
     * lift() ;
     * 
     * var topgoal = goal("g").toSolve((Integer k) -> k==32) . withTactic(a0) .
     * lift() ; agent.setGoal(topgoal); agent.update(); assertTrue(state.result==32)
     * ; assertTrue(topgoal.getStatus().success()) ; assertTrue(agent.goal == null)
     * ; }
     * 
     */
