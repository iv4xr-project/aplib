...
public abstract class LTL {
	...
	/**
	 * Abstractly representing the finite execution on which this LTL is interpreted.
	 */
	...
	public static class Not extends LTL {
		...
		LTLVerdict sat() {
			...
			while (iterator.hasNext()) {
				...
				switch (p) {
				case SAT : psi.verdict = LTLVerdict.UNSAT ; break ;
				case UNSAT : psi.verdict = LTLVerdict.SAT ; break ;
				}
			}
	...
	public static class And extends LTL {
		...
		LTLVerdict sat() {
			...
			while (iterator.hasNext()) {
				...
				if (allSat) psi.verdict = LTLVerdict.SAT ;
				else psi.verdict = LTLVerdict.UNSAT ;
			}
			return absexecution.getFirst().verdict ;
		}
		...
		LTLVerdict sat() {
			...
			while (iterator.hasNext()) {
				...
				if (q == LTLVerdict.SAT) {
					...
					if (nextSat && p == LTLVerdict.SAT) psi.verdict = LTLVerdict.SAT ;
					else {
	...
	public static class Next extends LTL {
		...
		LTLVerdict sat() {
			...
			while (iterator.hasNext()) {
				...
				switch(q) {
				  case SAT   : psi.verdict = LTLVerdict.SAT ; break ;
				  case UNSAT : psi.verdict = LTLVerdict.UNSAT ;
				}
			}
