 ...
 * There are several types of observations, such as verdict and coverage-event,
 * represented by different subclasses of this class.
 * 
 * @author Wish
 *
 */
public class ObservationEvent implements Serializable, Parsable {
	...
	public ObservationEvent(String family) { 
		if (family==null) throw new IllegalArgumentException("Trying to create an ObservationEvent with a null id.") ;
	 ...
	 * Of course testing cannot completely guarantee absence of bugs, but we can still measure
	 * relative completeness with respect to a set of coverage-checkpoint (or simply coverage-point), 
	 * each representing
	 * certain family of behaviors of the program-under-test. We can think this check-point as
	 * a certain point in the execution of the program-under-test. All executions that visit
	 * this check-point is thus the family of the behavior that the check-point represents.
	 * 
	 * A set of tests (also called a
	 * test-suite) is said to be relatively complete/adequate if the tests in this set 
	 * activates/visits the coverage-points that we set out for the program-under-test.
	 * 
	 * <p>The tester/developer should determine what he/she which coverage points to
	 * cover when testing a given program-under-test. There should also be a way for a test-agent
	 * to observe that it visits such a coverage point. When this happen, the test agent
	 * can report this as an instance of this class CoveragePointEvent.
	 * 
	 * @author Wish
	 *
	 */
	...
	/**
	 * This class is used to represent observation-events that need to 
	 * be time-stamped.
	 */
	public static class TimeStampedObservationEvent extends ObservationEvent {
		 ...
		 * 
		 * @param familyName  A name to classify this event as belonging to the said family.
	...
	/**
	 * When a test-agent interacts with a system-under-test, it will want to
	 * report things that meet its expectations, as well as things that violate
	 * its expectations (and may thus imply bugs). Such discoveries can be 
	 * reported as instances of this class VerdictEvent.
	 * 
	 */
	public static class VerdictEvent extends TimeStampedObservationEvent { 
		
		/**
		 * A "true" represents a positive verdict, which should be given 
		 * when a test-agent observes something that is correct, as it should be. 
		 * A "false" on the other hand, represents
		 * a negative verdict, which should be given when the test-agent 
		 * observes something that violates correctness. 
		 * Null represents undecided.
		 */
		 ...
		 * @param familyName   A name to classify the verdict-family this verdict can be said to belong to.
		 * @param info Additional descriptive string explaining the verdict.
		 * @param v    true if the verdict is positive, false if negative, null if undecided.
		 */
		...
		public VerdictEvent parse(String s) {
			...
			else if (v.equals("true")) o.verdict = true ;
