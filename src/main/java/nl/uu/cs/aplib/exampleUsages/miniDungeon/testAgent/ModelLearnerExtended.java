...
public class ModelLearnerExtended {
	...
	String HEAL = "" + EntityType.HEALPOT ;
	String RAGE = "" + EntityType.RAGEPOT ;
	...
	boolean isHealPot(WorldEntity e) {
		return e.type.equals(HEAL) ;
	}
	
	boolean isRagePot(WorldEntity e) {
		return e.type.equals(RAGE) ;
	}
	...
	public void learn(MyAgentState state, GameWorldModel model) {
		...
		for(WorldEntity e : state.worldmodel.elements.values()) {
			...
			if (!isScroll(e) && !isShrine(e) && !isFrodo(e) && !isSmeagol(e) && !isHealPot(e) && !isRagePot(e) ) continue ;
