 ...
 * 
 * Do note that an AutonomousBasicAgent requires a state of type {@link StateWithMessanger} .
 * 
 * @author Wish
 *
 */
public class AutonomousBasicAgent extends BasicAgent {
	...
	/**
	 * The time interval in ms between two ticks. This agent will strive to time
	 * the next tick to be invoked {@code samplingInterval}-ms after the start of
	 * the current tick. The accuracy of this timing depends on Java implementation
	 * of timer. If the current tick executes longer than {@code samplingInterval}-ms
	...
	/**
	 * If the agent is already running autonomously, invoking this method will cause it
	 * to pause. While it is paused, it will not trigger any tick. User {@link #resume()}
	 * to make the agent resuming its autonomous run. An incoming message will also cause
	 * the agent to resume.
	 */
