...
/**
 * 
 * A Tactic is needed to solve a {@link Goal}. There are the following
 * types of tactics:
 * 
 * <ol>
 *    <li> A PRIMITIVE tactic, consist of just a single {@link Action}.
 *    If invoked, this tactic will execute the Action, if the latter is
 *    enabled in the current agent's state.
 * 
 *    <li> A tactic T of the type FIRSTOF(T1,T2,...) where T1,T2,.. are
 *    tactics. Executing T will execute <b>the first</b> subtactic (so, T1 or
 *    T2 or ...) in the given order that has an enabled action to be executed
 *    in the current state.
 *         
 *    <li> A tactic T of the type ANYOF(T1,T2,...) where T1,T2,.. are
 *    tactics. Executing T will execute one of the subtactic
 *    that has an enabled action to be executed
 *    in the current state.
 *        
 *    <li>  A tactic T of the type SEQ(T1,T2,T3,...) where T1,T2,.. are
 *    tactics. Executing T will execute the subtactics in sequence.
 *    Note however that an agent (instance of {@link SimpleAgent}) always executes
 *    a tactic one action per tick. For example, if T1 is a SEQ of three
 *    actions, and T2 is an ANYOF two actions, and T3 is a single action,
 *    then T2 will execute at the 4th tick, and T3 at the 5th tick.
 *    
 * </ol>   
 * 
 * @author Wish
...
public class Tactic {
	 ...
	 */
	static public enum TacticType { FIRSTOF, ANYOF, SEQ, PRIMITIVE } 
	...
	/**
	 * Given a state, this method returns the set of actions in this tactic
	...
	List<PrimitiveTactic> getFirstEnabledActions(SimpleState agentstate) {
		...
		switch(strTy) {
		   case FIRSTOF : for (Tactic PT : subtactics) {
				             actions = PT.getFirstEnabledActions(agentstate) ;
				             if (! actions.isEmpty()) return actions ;
			              } 
			              return actions ;
		   case ANYOF   : for (Tactic PT : subtactics) {
				             actions.addAll(PT.getFirstEnabledActions(agentstate)) ;
			              }
			              return actions ;
		   case SEQ : return subtactics.get(0).getFirstEnabledActions(agentstate) ;
		   case PRIMITIVE : var this_ = (PrimitiveTactic) this ;
	...
	/**
	 * Suppose this tactic is done/completed. This method calculates the next
	 * tactic to execute. 
	 * 
	 * <p>Note: if the top tactic consists of only FIRSTof and ANYof
	 * nodes, and no actions are persistent, this method should return null. Else the next
	 * tactic is determined by the presence of SEQ and uncompleted persistent
	 * actions.
	 */
	Tactic calcNextTactic() {
	    ...
	    switch(parent.strTy) {
	       case FIRSTOF : return parent.calcNextTactic() ;
	       case ANYOF   : return parent.calcNextTactic() ;
	       case SEQ     : int k = parent.subtactics.indexOf(this) ;
	...
	/**
	 * Print some basic statistics of this tactic (e.g. the number of times
	 * each Action in this tactic has been invoked, and its total running time)
	 * to the console.
	 */
	...
	/**
	 * A subclass of {@link Tactic} representing a single {@link Action}. It bassically
