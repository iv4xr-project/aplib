...
/**
 * A simple game as an example for agents to play. The game is played by two players
 * on an NxN board. The players take turn to put their pieces. The pieces of player-1
...
public class FiveGame {
	...
	public FiveGame(int size, int numOfBlocked) {
		...
		while(numOfBlocked>0) {
			...
			if (board[x][y] == SQUARE.BLOCKED) continue ;
	...
	public GAMESTATUS getGameStatus() {
		...
		if (hasEmpty) return GAMESTATUS.UNFINISHED ;
		else return GAMESTATUS.TIE ;
	}
	
	/**
	 * Place a piece of the specified type in the given coordinate. If the square is unoccupied,
	...
	public String toString() {
		...
		for(int x=0; x<boardsize; x++) {
			s += "\n " ;
			for(int y=0; y<boardsize; y++)
				if (board[x][y] == SQUARE.EMPTY) s += " " ;
				else if (board[x][y] == SQUARE.CROSS) s += "X" ;
				else if (board[x][y] == SQUARE.CIRCLE) s += "O" ;
				else s += "#" ;
		}
	   s += "\n" + line(boardsize+2) ;		
	   return s ;
	...
	public String toStringShort() {
		...
		for (int x = 0 ; x<boardsize; x++)
			for(int y=0; y<boardsize; y++) {
				if (board[x][y] == SQUARE.CROSS) crosses++ ;
				else if (board[x][y] == SQUARE.CIRCLE) circles++ ;
				else if (board[x][y] == SQUARE.EMPTY) empty++ ;
				else block++ ;
			}
		var s = "" ;
		s += "(" + boardsize + "x" + boardsize + ") " + crosses + " X, "
			  + circles + " O, "
			  + block + " #, "
			  + empty + " empties. Status: " + getGameStatus() ;
		return s ;
	}
	
	public void print() { System.out.println(toString()) ; }
	public void printStatus() { System.out.println(toStringShort()) ; }
	...
	static public class RandomPlayer {
		 ...
		 */
		public RandomPlayer(SQUARE ty, FiveGame game) { this.ty = ty ; this.game = game ;}
		...
		List<int[]> getNeigbors(int x, int y) {
			...
			for (int x_ = xlow; x_ <= xhigh ; x_++) 
				for (int y_ = ylow; y_ <= yhigh ; y_++) {
					int[] position = {x_,y_} ;
					if (x_ != x && y_ != y) neighbors.add(position) ;
				}
			return neighbors ;
		}
		
		/**
		 * Find a random empty place, which is adjacent to another of the same type, and put
		 * a piece there. If there is no such square then choose a random empty square.
		 * If there is no empty square left, the method returns false.
		 */
		public boolean move() {
					...
					if(game.board[x][y] == SQUARE.EMPTY) {
						...
						for (int[] neighbor : getNeigbors(x,y)) {
							...
							if (game.board[x_][y_] == ty) {
								emptiesAdjacent.add(position) ; break ;
							}
						}
					}
				}
